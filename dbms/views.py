...
def get_searched_papers(request):
    if request.method=='POST':
        ...
        paper_list = []
        if papername == "None":
            paper_data = Paper.objects.filter(Q(broadDomain1=broadDomain) | Q(broadDomain2=broadDomain))
        else:
            paper_data = Paper.objects.filter(Q(broadDomain1=broadDomain) | Q(broadDomain2=broadDomain)).filter(title__contains=papername)
